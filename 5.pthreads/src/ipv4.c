...
void ipv4_outgoing(struct netdev *netdev, struct eth_hdr *hdr) {
    ...
    char src_addr_str[30];
    char dest_addr_str[30];
    inet_ntop(AF_INET, &iphdr->saddr, src_addr_str, 30);
    inet_ntop(AF_INET, &iphdr->daddr, dest_addr_str, 30);
    ...
    printf("Sucessfull IPv4_OUT Bout to tunnel it to ETH\n");
