...
static struct sock_buff *arp_alloc_skb() {
    struct sock_buff *skb = alloc_skb(ETH_HDR_LEN + ARP_HDR_LEN + ARP_DATA_LEN);
...
void arp_rcv(struct sock_buff *skb) {
    ...
    if (!(netdev = netdev_get(arpdata->dip))) {
        ...
        //char str3[20];
        //inet_ntop(AF_INET, skb->netdev, str3, 30);
        //printf("WE ARE %s\n", str3);
        return;
    }
...
int arp_request(uint32_t sip, uint32_t dip, struct netdev *dev) {
    ...
    arph = (struct arp_hdr *) skb_push(skb, ARP_HDR_LEN);

    arph->opcode = htons(ARP_REQUEST);
    arph->hwtype = htons(ARP_ETHERNET);
    arph->protype = htons(ETH_P_IP);
