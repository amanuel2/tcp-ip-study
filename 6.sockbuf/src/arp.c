...
void arp_incoming(struct netdev *netdev, struct eth_hdr *hdr)
{
    ...
    if (netdev->addr != arpdata->dip) {
        char str[30];
        //netdev->addr = htonl(netdev->addr);
        inet_ntop(AF_INET, &netdev->addr, str, 30);
        
        char str2[30];
        //arpdata->dip = htonl(arpdata->dip);
        inet_ntop(AF_INET, &arpdata->dip, str2, 30);
        printf("ARP was not for us. We are %s, They are %s\n", str, str2);
        return;
    }
...
int neigh_resolve_output(struct sock_buff *skb) {
    return 0;
}